...
import { addLiveEvent, addNewArtist } from '../../actions';
import ReactNative, {
  StyleSheet,
  Text,
  TouchableOpacity,
...
export class AddArtist extends Component {
  ...
  onPressConfirmButton(e) {
    if (this.state.name) {
      ...
      if (isNew(this.props.artists, this.state.name)) {
        this.props.onAddNewArtist({
            ...
            <TextInput
              ...
              {(() =>{
                if (this.state.requiredMessageVisible) {
                  return (
                    <View>
                      <Text style={{color: 'red'}}>アーティスト名は入力必須です。</Text>
                    </View>
                  )
                }
              })()}
...
const mapDispatchToProps = dispatch => {
  return {
    onAddNewArtist: artist => dispatch(addNewArtist(artist)),
  };
};
